...
class InsanKatilimci(Katilimci):
    def hamle_yap(self):
        hamle = input("Göl, Dağ, veya Nehir seçin: ").lower()
        while hamle not in ['göl', 'dağ', 'nehir']:
            hamle = input("Geçersiz seçim! Göl, Dağ, veya Nehir seçin: ").lower()
        return hamle

class RastgeleBilgisayarKatilimci(BilgisayarKatilimci):
    def hamle_yap(self):
        return random.choice(['göl', 'dağ', 'nehir'])
...
class Yarisma:
    ...
    def kazanan_belirle(self, insan_hamle, bilgisayar_hamle):
        ...
        elif (insan_hamle == 'göl' and bilgisayar_hamle == 'nehir') or \
             (insan_hamle == 'dağ' and bilgisayar_hamle == 'göl') or \
             (insan_hamle == 'nehir' and bilgisayar_hamle == 'dağ'):
