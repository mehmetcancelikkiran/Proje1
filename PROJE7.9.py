    ...
    def cihazlari_kontrol_et(self):
        while True:
            ...
            if islem == "aç":
                ...
                cihaz.durum_bilgisi()
            elif isinstance(cihaz, Isıtıcı) and islem == "sıcaklık":
                yeni_sicaklik = int(input("Yeni sıcaklığı girin: "))
                cihaz.sicaklik_ayarla(yeni_sicaklik)
            elif isinstance(cihaz, GüvenlikSistemi) and islem == "alarm":
...
if __name__ == "__main__":
    ...
    akilli_ev.cihaz_ekle(Lamba("Salon Lambası"))
    akilli_ev.cihaz_ekle(Isıtıcı("Salon Isıtıcı"))
    akilli_ev.cihaz_ekle(GüvenlikSistemi("Ev Güvenlik"))
